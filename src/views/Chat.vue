<template>
  <div class="chat" style="height: 100%;">
    <div
      id="scrollDiv"
      style="display: flex; flex-direction: column; align-items: flex-start; overflow: scroll;"
    >
      <div
        v-for="(key,i) in messages"
        :key="i"
        class="bubble"
        :class="{right: key.name === loggedInUser, left: key.name !== loggedInUser }"
      >
    ...
    <div
      class="fixed-bottom"
      style="height: 60px; width: 100%; background-color: white; display: flex; align-items: center"
    >
...
export default {
  ...
  updated() {
    document.getElementById("scrollDiv").scrollTop = document.getElementById(
      "scrollDiv"
    ).scrollHeight;
  },
  ...
  methods: {
    ...
    sendMessagesToFirebase() {
      ...
      firebase
        .database()
        .ref("chat")
        .push(message); //push the message to firebase
